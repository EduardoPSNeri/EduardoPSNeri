# Olá, eu sou Eduardo Neri 👋

💻 Desenvolvimento de Software | Back-End | Front-End | Full Stack  
🎓 Estudante de Análise e Desenvolvimento de Sistemas — 3º semestre de 4  
🛠️ Experiência profissional em Suporte e Infraestrutura de TI  
📍 Patos de Minas - MG, Brasil  

---

## Sobre mim

Sou profissional de Tecnologia da Informação e estudante de Análise e Desenvolvimento de Sistemas.

Atualmente desenvolvo projetos com foco em desenvolvimento de software, APIs REST, banco de dados, Programação Orientada a Objetos e desenvolvimento web.

Tenho experiência profissional com suporte técnico, infraestrutura, redes e atendimento remoto e presencial, o que contribuiu para desenvolver habilidades em resolução de problemas, análise técnica, organização e autonomia.

Busco evoluir profissionalmente em Desenvolvimento de Software, atuando em Back-End, Front-End ou Full Stack.

---

## Tecnologias

### Desenvolvimento
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)

### Banco de Dados e Ferramentas
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)
![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat&logo=sqlite&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)

---

## Projetos em destaque

### 🚗 FleetFlow

Sistema de gerenciamento de frotas desenvolvido com **Python, FastAPI, SQLite e arquitetura em camadas**.

Principais recursos:

- Cadastro e gerenciamento de veículos
- Cadastro de motoristas
- Controle de abastecimentos
- Diário de bordo
- Controle de manutenções
- Atualização de quilometragem
- Regras de negócio e validações
- API REST com FastAPI
- Arquitetura Router → Service → Repository

🔗 [Ver projeto](https://github.com/EduardoPSNeri/fleetflow)

---

### 📦 Sistema de Controle de Estoque

Aplicação em Python para gerenciamento de produtos e estoque, desenvolvida com foco em organização de código e aplicação de conceitos de desenvolvimento.

Principais conceitos aplicados:

- Programação Orientada a Objetos
- Persistência de dados com JSON
- Validação de dados
- Separação de responsabilidades
- Modularização
- Testes automatizados com Pytest

🔗 [Ver projeto](https://github.com/EduardoPSNeri/sistema-estoque-python)

---

## GitHub Stats

![Eduardo's GitHub Stats](https://github-readme-stats.vercel.app/api?username=EduardoPSNeri&show_icons=true&hide_title=true)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=EduardoPSNeri&layout=compact)

---

## Contato

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Eduardo%20Neri-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/eduardo-neri-96b3732a5/)

[![GitHub](https://img.shields.io/badge/GitHub-EduardoPSNeri-181717?style=flat&logo=github&logoColor=white)](https://github.com/EduardoPSNeri)
